...
void setup(int* numPlayers, struct Player** profiles, struct Overall** overall) {
	...
	*profiles = malloc(*numPlayers*sizeof(struct Player));
	...
	// Allocate space for overall game status
	*overall = malloc(sizeof(struct Overall));
...
void display(int numPlayers, struct Player* profiles, struct Overall* overall) {
	...
	while (!back) {
		printf("\nDisplay Options:\n"
			...
			"	e.g. show Raj\n"
			"4. Enter \"rolls\" to graph all recorded rolls, check last roll, and test normality.\n"
			"5. Enter \"back\" or \"b\" to return to the previous menu.\n\n");
		...
		if (strcmp(command, "list") == 0) {
			...
			puts(graph);
		} else if (strcmp(command, "back") == 0 || strcmp(command, "b") == 0) {
			back = true;
		} else {
			printf("Invalid command.\n");
		}
...
void save(int numPlayers, struct Player* profiles, struct Overall* overall) {
	...
	fprintf(csv, "Name, Roads, Settlements, Cities, Development Cards, Knight Cards\n");
	for (int i = 0; i < numPlayers; i++) {
		fprintf(csv, "%s, %d, %d, %d, %d, %d\n", profiles[i].name, profiles[i].roads, profiles[i].settlements, \
			profiles[i].cities, profiles[i].developmentCards, profiles[i].knightCards);
	}
	fprintf(csv, "\nRolls\n");
	for (int i = 0; i < 10; i++) {
		fprintf(csv, "%d, ", overall->rolls[i]);
	}
	fprintf(csv, "%d", overall->rolls[10]);
	printf("\nSave complete.\n");
...
void menu(int numPlayers, struct Player* profiles, struct Overall* overall, bool* quit) {
	printf("\nMenu:\n"
		...
		"2. Enter \"display\" to display recorded data.\n"
